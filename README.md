...
Пошаговый план реализации системы автоматического распознавания и обработки автомобильных номеров согласно ТЗ. После завершения каждого шага статус должен обновляться в этом документе.
...
- [ ] Шаг 1: Базовая архитектура и каркас сервисов
  - Описать целевую архитектуру модулей (ingest, decoder, detector, tracker, OCR, postprocess, rules engine, events, webhooks, relay, admin/API/UI, monitoring).
  - Подготовить каркас репозитория: backend (FastAPI/Go), frontend (Vue/React), общие пакеты.
  - Определить интерфейсы между модулями и форматы обмена данными.
- [ ] Шаг 2: Инфраструктура хранения и конфигурации
  - Спецификация схемы БД PostgreSQL и структуры объектного хранилища S3/MinIO.
  - Настройка миграций, подключение конфигурации (env, secrets), базовая модель пользователей и ролей.
...
- [ ] Шаг 8: API и авторизация
  ...
  - JWT/OAuth2, RBAC (admin/operator/viewer), аудит изменений настроек, TLS для внешних интерфейсов.
